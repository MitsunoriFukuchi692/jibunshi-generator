...
export default function UserPage({ userId, setUserId }: { userId: number | null; setUserId: (id: number | null) => void }) {
  // ステップ管理
  ...
  const handleRegister = async () => {
    try {
      const response = await fetch(`${import.meta.env.VITE_API_URL}/api/users`, {
        ...
        body: JSON.stringify({ name: userName, age: userAge, birth_date: birthDate, gender, address, occupation, bio }),
      });
      if (!response.ok) throw new Error('登録に失敗しました');
      const data = await response.json();
      alert(`${userName}さんが登録されました！ ID: ${data.id}`);
      setUserId(data.id);
      setStep(4);
    } catch (error) {
  ...
  const renderStep5 = () => (
    <>
      {userId && <PhotoUploadPage userId={userId} onComplete={() => setStep(6)} />}
    </>
  );

  // ステップ6: 次のステップ（ターニングポイント入力へ）
  const renderStep6 = () => (
    <>
      <div style={styles.info}>
        ✓ 写真のアップロードが完了しました！
  ...
  const renderStep7 = () => (
    <>
      {userId && <TurningPointPage userId={userId} birthDate={birthDate} onComplete={() => setStep(8)} />}
    </>
  );
  ...
  const renderStep8 = () => (
    <>
      <div style={styles.completionMessage}>
        <div style={styles.completionIcon}>🎉</div>
        <div style={styles.completionText}>ターニングポイントの入力が完了しました！</div>
        <div style={{ fontSize: '16px', color: '#27ae60' }}>
  ...
  const renderStep9 = () => (
    <>
      {userId && <InterviewPage userId={userId} />}
    </>
  );
